...
export function RequiredSavingsCard({
  ...
  return (
    <Card className="flex flex-col">
      <div className="text-slate-500 dark:text-slate-400 text-sm font-medium mb-1">Ahorro Necesario (Entrada + Gastos)</div>
      <div className="text-2xl font-bold text-slate-900 dark:text-slate-100">{formatCurrency(totalInitialCash)}</div>
      ...
      <div className="mt-auto pt-4 flex-1 flex flex-col justify-end">
        ...
        <div className="bg-slate-50 dark:bg-slate-800/50 p-2.5 rounded-none border border-slate-100 dark:border-slate-700 text-xs">
          <div className="flex justify-between items-center mb-1.5">
            ...
            <span className="font-medium text-slate-700 dark:text-slate-200">{formatCurrency(downPayment)}</span>
          </div>
          <div className="flex justify-between items-center mb-1.5">
            <span className="text-slate-500 dark:text-slate-400">Gastos e impuestos:</span>
            <span className="font-medium text-slate-700 dark:text-slate-200">{formatCurrency(totalExpenses)}</span>
          </div>

          {savings > 0 ? (
            <>
              <div className="flex justify-between items-center border-t border-slate-200 dark:border-slate-700 pt-1.5 mt-1.5">
                <span className="text-slate-500 dark:text-slate-400 font-medium">Falta por ahorrar:</span>
                <span className="font-semibold text-amber-600 dark:text-amber-400">{formatCurrency(missingSavings)}</span>
              </div>

              {missingSavings > 0 && monthlySavings > 0 && (
                <div className="border-t border-slate-200 dark:border-slate-700 pt-1.5 mt-1.5">
                  <div className="text-slate-500 dark:text-slate-400 mb-0.5">Tiempo estimado para ahorrar lo que falta:</div>
                  <div className="font-semibold text-slate-700 dark:text-slate-200">{monthsToSave} meses <span className="font-normal text-slate-500 dark:text-slate-400">({yearsToSave} años)</span></div>
                </div>
              )}
            </>
          ) : (
