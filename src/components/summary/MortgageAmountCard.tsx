...
export function MortgageAmountCard({
  ...
  return (
    <Card className="flex flex-col">
      <div className="text-slate-500 text-sm font-medium mb-1">Importe Hipoteca</div>
      <div className="text-2xl font-bold text-slate-900">{formatCurrency(loanAmount)}</div>
      ...
      <div className="mt-auto pt-4 flex-1 flex flex-col justify-end">
        <div className="bg-slate-50 p-2.5 rounded-none border border-slate-100 text-xs">
          <div className="flex justify-between items-center mb-1.5">
            ...
            <span className="font-medium text-slate-700">{ltv.toFixed(1)}%</span>
          </div>
          <div className="flex justify-between items-center mb-1.5">
            <span className="text-slate-500">Intereses totales:</span>
            <span className="font-medium text-amber-600">{formatCurrency(totalInterestPaid)}</span>
          </div>
          <div className="flex justify-between items-center border-t border-slate-200 pt-1.5 mt-1.5">
            <span className="text-slate-500 font-medium">Coste Total Operación:</span>
            <span className="font-bold text-indigo-700">{formatCurrency(totalCostOfProperty)}</span>
          </div>
