        ...
        <div className="bg-slate-50 dark:bg-slate-800/50 p-2.5 rounded-none border border-slate-100 dark:border-slate-700 text-xs">
          ...
          <div className="flex justify-between items-center border-t border-slate-200 dark:border-slate-700 pt-1.5 mt-1.5">
            ...
            <span className="font-bold text-indigo-700 dark:text-indigo-400">
              {formatCurrency(totalCostOfProperty)}
            </span>
          </div>
          ...
          <div className="mt-2">
            <div className="h-1.5 w-full rounded-none flex overflow-hidden mb-2">
              <div
                ...
                style={{ width: `${((loanAmount + downPayment) / totalCostOfProperty) * 100}%` }}
              ></div>
              <div
                className="bg-amber-500 h-full"
                style={{ width: `${(totalInterestPaid / totalCostOfProperty) * 100}%` }}
              ></div>
              <div
                className="bg-red-500 h-full"
                style={{ width: `${(totalExpenses / totalCostOfProperty) * 100}%` }}
              ></div>
              <div
                className="bg-emerald-500 h-full"
                style={{
                  width: `${((monthlyIbiAndCommunity * 12 * years) / totalCostOfProperty) * 100}%`,
                }}
              ></div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-x-1 gap-y-1 text-xs text-slate-500 dark:text-slate-400">
              <div className="flex items-center gap-1">
                <div className="w-1.5 h-1.5 rounded-none bg-blue-500"></div>Inmueble (
                {(((loanAmount + downPayment) / totalCostOfProperty) * 100).toFixed(0)}%)
              </div>
              <div className="flex items-center gap-1">
                <div className="w-1.5 h-1.5 rounded-none bg-amber-500"></div>Intereses (
                {((totalInterestPaid / totalCostOfProperty) * 100).toFixed(0)}%)
              </div>
              <div className="flex items-center gap-1">
                <div className="w-1.5 h-1.5 rounded-none bg-red-500"></div>Impuestos (
                {((totalExpenses / totalCostOfProperty) * 100).toFixed(0)}%)
              </div>
              <div className="flex items-center gap-1">
                <div className="w-1.5 h-1.5 rounded-none bg-emerald-500"></div>IBI/Com. (
                {(((monthlyIbiAndCommunity * 12 * years) / totalCostOfProperty) * 100).toFixed(0)}%)
              </div>
            </div>
          </div>
        </div>
